...
def _format_hot_line(topic, width, show_translation=False):
    """
    Гармоничная раскладка TOP BREAKING:
      22:30  ⚡  [РИА]           Заголовок обычным цветом
             ↑    ↑              ↑
          время  маркер  источник
    """
    inner = width - 2
    item = topic["item"]
    title = pick_title(item, show_translation)
    ...
    marker_w = 2
    source_w = 14
    title_w = inner - time_w - 2 - marker_w - 1 - source_w - 1
    if title_w < 10:
        title_w = 10
    title_fit = fit_text(title, title_w)

    time_part = HOT_TIME_COLOR + update_time + Color.RESET
    marker_part = marker_color + marker + Color.RESET
    source_part = HOT_SOURCE_COLOR + pad_visible(source, source_w) + Color.RESET
    title_part = HOT_TITLE_COLOR + pad_visible(title_fit, title_w) + Color.RESET
    ...
    if visible_width(line) != width:
        return box_line(
            "{} {} {} {}".format(update_time, marker, source, title),
            width)
    return line
...
def build_frame_lines(history, width, height, source_health,
    ...
    stream = [
        ...
        if x.get("key") not in hot_item_keys
        and x.get("level") == "NORMAL"
    ]
